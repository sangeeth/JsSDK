 ...
 */
package net.sangeeth.jssdk.dwr;
...
{
	private static final String JSSDK_PACKAGE_NAME= "js.dwr.interface.";
    ...
    public void handle(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
    {
        ...
        out.printf("%s._path = '%s';",classname,path); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        for (int i = 0; i < methods.length; i++)
        {
            ...
            out.printf("\n%s.%s = function(",classname,methodName); //$NON-NLS-1$
            ...
            out.printf("    js.dwr.core.DWREngine._execute(%s._path, '%s', '%s\', ",classname,classname,methodName); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            for (int j = 0; j < paramTypes.length; j++)
            {
                if (LocalUtil.isServletClass(paramTypes[j]))
                {
